import React from 'react'
import type { NoteItem } from '../types/dashboardTypes'

type NoteCardProps = Omit<NoteItem, "id"> & {
  icon?: React.ReactNode;
};

const NoteCard = ({ status, title, description, date, meta, tags = [], icon }: NoteCardProps) => {
  const statusClass = status === "Completed" ? "bg-emerald-50 text-emerald-700 border-emerald-100" : "bg-slate-50 text-slate-700 border-slate-100";

  return (
    <article className="bg-white rounded-2xl border border-slate-200 shadow-md p-5 h-full flex flex-col">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          <div className="h-8 w-8 rounded-xl bg-slate-100 flex items-center justify-center text-slate-600">
            {icon ?? <span className="text-xs font-bold">N</span>}
          </div>
          <span className={["text-xs font-semibold px-2.5 py-1 rounded-full border", statusClass].join(" ")}>{status}</span>
        </div>
      </div>

      <h4 className="mt-2 text-sm font-semibold text-slate-900 line-clamp-1">{title}</h4>
      <p className="mt-2 text-sm text-slate-600 line-clamp-2">{description}</p>

      {tags.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-2">
          {tags.slice(0, 3).map((t) => (
            <span key={t} className="text-xs font-medium text-slate-600 bg-slate-50 border border-slate-200 rounded-full px-2.5 py-1">#{t}</span>
          ))}
          {tags.length > 3 && <span className="text-xs text-slate-500">+{tags.length - 3}</span>}
        </div>
      )}

      <div className="mt-auto pt-4 flex items-center justify-between text-xs text-slate-500">
        <span>{date}</span>
        {meta && <span>{meta}</span>}
      </div>
    </article>
  )
}

export default NoteCard