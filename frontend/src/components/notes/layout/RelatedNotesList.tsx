  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-3 divide-y divide-slate-100">
        ...
        {notes.map((n) => (
          <Link key={n.id} to={`/notes/${n.id}`} className="w-full flex items-center justify-between py-3 text-left bg-slate-50 px-2 rounded-xl transition mb-2 hover:bg-slate-100">
            ...
            <HiChevronRight className="text-slate-400 text-lg shrink-0" />
          </Link>
        ))}
      </div>
    </div>  
